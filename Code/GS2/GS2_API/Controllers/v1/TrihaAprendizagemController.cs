...
namespace GS2_API.Controllers.v1
{
    ...
    [Route("api/trilha")]
    [ApiController]
    public class TrilhaAprendizagem : ControllerBase
    {
        private readonly TrilhaAprendizagemService _service;

        public TrilhaAprendizagem(TrilhaAprendizagemService service)
        {
            _service = service;
        }

        [HttpPost]
        public async Task<IActionResult> Criar(TrilhaAprendizagemDto dto)
        {
            if (dto == null)
        ...
        [HttpPut("{id:int}")]
        public async Task<IActionResult> Atualizar(int id, TrilhaAprendizagemDto dto)
        {
            if (dto == null)
                return BadRequest("Payload inválido.");
